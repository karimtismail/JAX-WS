...
public class BaseEntityRepositoryImpl<T, ID> implements BaseEntityRepository<T, ID> {
     ...
     */
    @Override
    public T findById(ID id, EntityManager entityManager) throws Exception {
        try {
            return entityManager.find(entityClass, id);
        } catch (Exception ex) {
            throw new Exception(entityClass.getSimpleName() + " id " + id + " is not exist into our database");
        }
