...
public class BaseService<T, ID, DR, DS> {
    ...
    public DS findById(ID id) {
        return transactionalEntityManager.executeInTransaction(entityManager -> {
            T entity = null;
            try {
                entity = repository.findById(id, entityManager);
            } catch (Exception e) {
     ...
     * @return The corresponding Data Transfer Object for response.
     * @throws Exception                if the entity with the specified identifier is not found.
     * @throws IllegalArgumentException if the provided DTO request object is null.
     */
    public DS update(ID id, DR dtoRequest) throws Exception {
        if (dtoRequest == null) {
        ...
        var entityTrans = transactionalEntityManager.executeInTransaction(
                entityManager -> {
                    try {
                        return repository.findById(id, entityManager);
                    } catch (Exception e) {
                        throw new RuntimeException(e);
                    }
                }
     ...
     * @throws Exception if the entity with the specified identifier is not found.
     */
    public DS deleteById(ID id) throws Exception {
        var entityTrans = transactionalEntityManager.executeInTransaction(
                entityManager -> {
                    try {
                        return repository.findById(id, entityManager);
